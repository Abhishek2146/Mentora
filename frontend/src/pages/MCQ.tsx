  ...
  if (done) return (
    <AppLayout title="MCQ Practice">
      <div className="max-w-xl mx-auto card p-10 flex flex-col items-center gap-6 text-center">
        <p className="text-5xl font-black gradient-text">{Math.round((score/questions.length)*100)}%</p>
              ...
              {q.options?.map((opt: string) => (
                <button key={opt} onClick={() => !selected && setSelected(opt)}
                  className={`w-full text-left p-4 rounded-xl border-2 text-sm font-medium transition-all ${
                    selected === opt && opt === q.correct_answer ? "border-emerald-500 bg-emerald-50" :
