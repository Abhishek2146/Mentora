    ...
    <AppLayout title="Voice Learning">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="card p-8 flex flex-col items-center gap-6 text-center">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Voice Learning Mode</h2>
          ...
          <div className="space-y-2">
            {["Explain what normalization is", "What are the ACID properties?", "How does a B+ tree work?", "Quiz me on SQL joins"].map(p => (
              <div key={p} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                ...
                <span className="text-sm text-slate-600 dark:text-slate-300 italic">"{p}"</span>
              </div>
            ))}
          </div>
        </div>
